...
int tplg_create_pga(struct tplg_context *ctx, struct sof_ipc_comp *comp)
{
	...
	comp->id = comp_id;
	comp->hdr.size = sizeof(struct sof_ipc_comp_volume) + UUID_SIZE;
	comp->type = SOF_COMP_VOLUME;
	comp->pipeline_id = ctx->pipeline_id;
	comp->ext_data_length = UUID_SIZE;
	volume->config.hdr.size = sizeof(struct sof_ipc_comp_config);
	memcpy((uint8_t *)comp + sizeof(*volume), &uuid, UUID_SIZE);
...
int tplg_register_pga(struct tplg_context *ctx)
{
	...
	comp = (struct sof_ipc_comp *)malloc(sizeof(*volume) + UUID_SIZE);
	if (!comp)
		return -ENOMEM;

	memset(comp, 0, sizeof(*volume) + UUID_SIZE);

	volume = (struct sof_ipc_comp_volume *)comp;

	ret = tplg_create_pga(ctx, comp);
	if (ret < 0)
		goto err;
