			 ...
			 struct output_stream_buffer *output_buffers, int num_output_buffers)
{
	...
	comp_info(dev, "mixer_copy() %d", num_input_buffers);

	/* too many sources ? */
	if (num_input_buffers == PLATFORM_MAX_STREAMS - 1)
		return 0;
